...
describe("filterBatchesByMonth", () => {
  ...
  it("filters a period settlement by its end date when it has no target month", () => {
    const batches = [
      {
        target_month: null,
        period_ended_at: "2026-05-31T12:00:00.000Z",
        id: "1",
      },
    ];
    expect(filterBatchesByMonth(batches, "2026-05")).toEqual(batches);
  });
});
