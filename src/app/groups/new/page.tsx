    ...
    <div className="flex flex-col gap-8">
      ...
      <div id="group-form" className="card-glass p-7">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold tracking-tight text-slate-900">新しいグループ</h2>
            <p className="mt-1 text-sm text-slate-500">作成するとあなたがオーナーとして参加します。</p>
          </div>
        </div>
        {error ? (
          <p className="mt-5 rounded-2xl border border-red-100 bg-red-50/80 px-4 py-3 text-sm text-red-800">{error}</p>
        ) : null}
        <form className="mt-7 flex flex-col gap-5" action={createGroupAction}>
          <label className="flex flex-col gap-2 text-sm">
            ...
            <input className="input-field" name="name" required placeholder="例: 山田家" />
          </label>
          <div className="flex flex-wrap gap-3">
            <FormSubmitButton label="作成" pendingLabel="作成中..." className="btn-primary" />
            <Link href={ROUTES.groups} className="btn-secondary">戻る</Link>
          </div>
        </form>
      </div>
    </div>
