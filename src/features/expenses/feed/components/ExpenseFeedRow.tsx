...
export function ExpenseFeedRow({ item, canEdit, onEdit, onView }: Props) {
  ...
  return (
    <div
      className="group flex items-center justify-between rounded-[24px] border border-slate-100 bg-white p-5 shadow-sm transition-all duration-200 hover:shadow-md active:bg-slate-50"
      role={item.viewable ? "button" : undefined}
      tabIndex={item.viewable ? 0 : undefined}
      onClick={item.viewable ? onView : undefined}
      onKeyDown={item.viewable ? (event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onView(); } } : undefined}
    >
